...
- **FR-002**: The system MUST update one or more top-level frontmatter fields in a single invocation, accepting scalar values and whole-list replacement of list-valued fields.
...
- Values supplied to an edit are interpreted as YAML scalars (unquoted `true`/`42` become boolean/number; quoting forces a string); the schema then enforces expected types. Exact command-line syntax for list values is a design-phase decision.
